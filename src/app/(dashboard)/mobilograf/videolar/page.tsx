...
export default async function VideolarPage() {
  const items = await getContentItems("videographer");
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-lg font-bold text-primary">Tayyor videolar</h1>
        <p className="text-sm text-secondary">
          Nashr etilgan videolar va ularning ko'rishlar soni
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        {published.length === 0 ? (
          <p className="text-sm text-muted">Hozircha nashr etilgan video yo'q</p>
        ) : null}
      </div>
    </div>
  );
